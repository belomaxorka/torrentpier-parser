 ...
 * @return array
 */
function rutrackerru($text)
{
	// ------------------- Get title -------------------
	preg_match_all('#<title>([\s\S]*?) :: (?=.*?RuTracker\.RU).*?</title>#', $text, $match);
	$title = $match[1];

	// ------------------- Get download link -------------------
	preg_match_all('#^<a href="dl\.php\?id=(\d+)"(?:\s+class="genmed")?>$#', $text, $match, PREG_SET_ORDER);
	$torrent = $match[1];
